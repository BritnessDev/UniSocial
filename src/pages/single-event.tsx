...
const SingleEvent: NextPageWithLayout = () => {
  ...
  const { data: event, isLoading: loading1 } = trpc.event.getEventById.useQuery({
    id: router.query.eid && typeof router.query.eid == "string" ? router.query.eid : '',
  })
  ...
  useEffect(() => {
    if (!loading1) {
      setLoading(false)
      // set Mock Data
      mockFeature['sglCard']['title'] = event?.title ? event.title : ''
      mockFeature['sglCard']['desc'][0] = event?.description ? event.description : ''
      mockFeature['sglContact']['location'] = event?.location ? event.location : ''
      mockFeature['sglContact']['price'] = event?.price ? event.price : 0
      mockFeature['photo'] = event?.imageUrls[0] ? event.imageUrls[0] : ''
